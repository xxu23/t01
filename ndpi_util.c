...
struct ndpi_workflow *ndpi_workflow_init(const struct ndpi_workflow_prefs
					 *prefs)
{
	...
	struct ndpi_workflow *workflow =
	    ndpi_calloc(1, sizeof(struct ndpi_workflow));

	workflow->prefs = *prefs;
	workflow->ndpi_struct = module;

	if (workflow->ndpi_struct == NULL) {
		NDPI_LOG(0, NULL, NDPI_LOG_ERROR,
			 "global structure initialization failed\n");
		exit(-1);
	}

	workflow->ndpi_flows_root =
	    ndpi_calloc(workflow->prefs.num_roots, sizeof(void *));
	return workflow;
}
...
void ndpi_workflow_free(struct ndpi_workflow *workflow)
{
	...
	for (i = 0; i < workflow->prefs.num_roots; i++)
		ndpi_tdestroy(workflow->ndpi_flows_root[i],
...
static struct ndpi_flow_info *get_ndpi_flow_info(struct ndpi_workflow *workflow,
						 ...
						 const struct ndpi_ipv6hdr
						 *iph6, u_int16_t ip_offset,
						 u_int16_t ipsize,
	...
	*proto = iph->protocol;
	...
	if (iph->protocol == 6 && l4_packet_len >= 20) {
		...
		*sport = ntohs((*tcph)->source), *dport = ntohs((*tcph)->dest);
		...
		if (iph->saddr < iph->daddr) {
			lower_port = (*tcph)->source, upper_port =
			    (*tcph)->dest;
		...
		*payload_len =
		    ndpi_max(0, l4_packet_len - sizeof(struct ndpi_udphdr));

		if (iph->saddr < iph->daddr) {
			lower_port = (*udph)->source, upper_port =
			    (*udph)->dest;
			*src_to_dst_direction = 1;
		} else {
			lower_port = (*udph)->dest, upper_port =
			    (*udph)->source;
	...
	if (0)
		NDPI_LOG(0, workflow.ndpi_struct, NDPI_LOG_DEBUG,
			 "[NDPI] [%u][%u:%u <-> %u:%u]\n", iph->protocol,
			 lower_ip, ntohs(lower_port), upper_ip,
			 ntohs(upper_port));

	idx =
	    (vlan_id + lower_ip + upper_ip + iph->protocol + lower_port +
	     upper_port) % workflow->prefs.num_roots;
	ret =
	    ndpi_tfind(&flow, &workflow->ndpi_flows_root[idx],
		       ndpi_workflow_node_cmp);

	if (ret == NULL) {
		if (workflow->stats.ndpi_flow_count ==
			...
			struct ndpi_flow_info *newflow =
			    ...
			    malloc(sizeof(struct ndpi_flow_info));

			if (newflow == NULL) {
				NDPI_LOG(0, workflow.ndpi_struct,
					 NDPI_LOG_ERROR,
			...
			newflow->protocol = iph->protocol, newflow->vlan_id =
			    vlan_id;
			newflow->lower_ip = lower_ip, newflow->upper_ip =
			    upper_ip;
			newflow->lower_port = lower_port, newflow->upper_port =
			    upper_port;
			newflow->ip_version = version;

			if (version == 4) {
				inet_ntop(AF_INET, &lower_ip,
					  newflow->lower_name,
					  sizeof(newflow->lower_name));
				inet_ntop(AF_INET, &upper_ip,
					  newflow->upper_name,
					  sizeof(newflow->upper_name));
			} else {
				inet_ntop(AF_INET6, &iph6->ip6_src,
					  newflow->lower_name,
					  sizeof(newflow->lower_name));
				inet_ntop(AF_INET6, &iph6->ip6_dst,
					  newflow->upper_name,
					  sizeof(newflow->upper_name));
			}

			if ((newflow->ndpi_flow =
			     ndpi_malloc(SIZEOF_FLOW_STRUCT)) == NULL) {
				NDPI_LOG(0, workflow.ndpi_struct,
					 NDPI_LOG_ERROR,
				...
				memset(newflow->ndpi_flow, 0,
				       SIZEOF_FLOW_STRUCT);

			if ((newflow->src_id =
			     ndpi_malloc(SIZEOF_ID_STRUCT)) == NULL) {
				NDPI_LOG(0, workflow.ndpi_struct,
					 NDPI_LOG_ERROR,
...
static struct ndpi_flow_info *get_ndpi_flow_info6(struct ndpi_workflow
						  *workflow, u_int16_t vlan_id, const struct ndpi_ipv6hdr
						  *iph6, u_int16_t ip_offset,
						  struct ndpi_tcphdr **tcph,
						  struct ndpi_udphdr **udph,
						  u_int16_t * sport,
						  ...
						  src_to_dst_direction)
{
	...
	if (iph.protocol == 0x3C /* IPv6 destination option */ ) {
		u_int8_t *options =
		    (u_int8_t *) iph6 + sizeof(const struct ndpi_ipv6hdr);
	...
	return (get_ndpi_flow_info(workflow, 6, vlan_id, &iph, iph6, ip_offset,
				   ...
				   src, dst, proto, payload, payload_len,
				   src_to_dst_direction));
}
				      ...
				      u_int16_t ipsize, u_int16_t rawsize)
{
	...
	//printf("flow->detection_completed=%d\n", flow->detection_completed);
	if (flow->detection_completed) {
		flow->detected_protocol =
		    ndpi_detection_process_packet(workflow->ndpi_struct,
						  ...
						  : (uint8_t *) iph6, ipsize,
						  time, src, dst);
		//printf("detected_protocol=%d %d %d\n", flow->detected_protocol.master_protocol, flow->detected_protocol.protocol, flow->packets);
		return (0);
	}

	flow->detected_protocol =
	    ndpi_detection_process_packet(workflow->ndpi_struct, ndpi_flow,
					  iph ? (uint8_t *) iph : (uint8_t *)
					  iph6, ipsize, time, src, dst);
	//printf("detected_protocol=(%d,%d) %d\n", flow->detected_protocol.master_protocol, flow->detected_protocol.protocol, flow->packets);
	...
	if ((flow->detected_protocol.protocol != NDPI_PROTOCOL_UNKNOWN)
		...
		if ((flow->detected_protocol.protocol == NDPI_PROTOCOL_UNKNOWN)
			...
			ndpi_set_detected_protocol(workflow->ndpi_struct,
						   ndpi_flow,
						   NDPI_PROTOCOL_STUN,
						   NDPI_PROTOCOL_UNKNOWN);

		snprintf(flow->host_server_name, sizeof(flow->host_server_name),
			 "%s", flow->ndpi_flow->host_server_name);
		...
		if (flow->detected_protocol.protocol == NDPI_PROTOCOL_UNKNOWN) {
			...
			if (flow->detected_protocol.protocol ==
				...
				if (workflow->__flow_detected_callback != NULL)
					workflow->__flow_detected_callback
					    (workflow, flow,
					     workflow->__flow_detected_udata,
					     workflow->__flow_packet_header,
					     workflow->__flow_packet_data);
			}
		} else {
			if (workflow->__flow_detected_callback != NULL)
				workflow->__flow_detected_callback(workflow,
								   flow,
								   workflow->__flow_detected_udata,
								   workflow->__flow_packet_header,
								   workflow->__flow_packet_data);
				  ...
				  const u_char * packet)
{
  ...
  /** --- IPv6 header --- **/
	...
	/* setting time */
	time =
	    ((uint64_t) header->ts.tv_sec) * TICK_RESOLUTION +
...
datalink_check:
	...
	case DLT_IEEE802_11_RADIO:
		...
		wifi =
		    (struct ndpi_wifi_header *)(packet + eth_offset +
						radio_len);
		fc = wifi->fc;
		...
		llc =
		    (struct ndpi_llc_header *)(packet + eth_offset + wifi_len +
					       radio_len);
		if (llc->dsap == SNAP)
		...
		ip_offset =
		    wifi_len + radio_len + sizeof(struct ndpi_llc_header) +
		    eth_offset;
		break;
	...
	case VLAN:
		vlan_id =
		    ((packet[ip_offset] << 8) + packet[ip_offset + 1]) & 0xFFF;
...
iph_check:
	...
	if (iph->version == 4) {
		...
		if (proto == 0x3C /* IPv6 destination option */ ) {
			...
			u_int8_t *options =
			    (u_int8_t *) & packet[ip_offset + ip_len];
			proto = options[0];
...
v4_warning:
	...
	if (workflow->prefs.decode_tunnels && (proto == IPPROTO_UDP)) {
		...
		if ((sport == GTP_U_V1_PORT) || (dport == GTP_U_V1_PORT)) {
			...
			u_int16_t encapsulates =
			    ntohs(*((u_int16_t *) & packet[offset + 2]));
